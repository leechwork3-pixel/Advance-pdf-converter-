...
async def conversion_callback(client, callback_query: CallbackQuery):
    ...
    try:
        await callback_query.message.edit_text(f"`Converting to {target_ext.upper()}...`")
        ...
        await callback_query.message.edit_text(f"**Conversion Failed!**\n\n**Error:** `{e}`")
